...
class HyperKvasirLiAugInput(CIFAR10Input):
    cfg = copy.deepcopy(CIFAR10Input.cfg)
    cfg.update(
        dict(
            data_dir=None,
            num_classes=23,
            multiclass=False,
            tfds_name='hyperkvasir_li/aug_0_2x',
            splits=dict(
                train=dict(num_images=26427, tfds_split='split_0'),
                minival=dict(num_images=5338, tfds_split='split_1'),
                eval=dict(num_images=5338, tfds_split='split_1'),
                trainval=dict(num_images=5338, tfds_split='split_1'),
            )))
...
def get_dataset_class(ds_name):
    return {
        ...
        'hyperkvasir_li_aug_0_2x': HyperKvasirLiAugInput
    }[ds_name]
...
class HyperKvasirLiAug02x:
    cfg = hparams.Config(
        data=dict(
            ds_name='hyperkvasir_li_aug_0_2x',
            multiclass=False,
        ),
        train=dict(
            batch_size=128,
...
@ds_register
class HyperKvasirLiFt4(HyperKvasirLi):
    """Finetune hyperkvasir configs."""
    # Finetune should have less regularization due to the limited training steps.
    cfg = hparams.Config(
        model=dict(
